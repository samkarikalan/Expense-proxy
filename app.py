...
- date: extract from receipt (e.g. 2026年04月18日 → 2026-04-18). Use today if not found.”””
  ...
  try:
  # FIX: use x-goog-api-key header instead of ?key= query param
  resp = requests.post(
  GEMINI_URL,
  headers={
  ...
  ```
    # FIX: capture full error body before raising
    if not resp.ok:
        return jsonify({ 'error': f'Gemini error {resp.status_code}: {resp.text}' }), 502
  ...
  return jsonify({ ‘error’: f’Gemini API error: {error_body}’ }), 502
